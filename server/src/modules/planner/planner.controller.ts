...
export async function buildDay(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const { tasks, conflicts, availableMinutes } = buildDayPlan({
      ...
      routineTaskDetails: Object.fromEntries((routineTasks || []).map((task: { routineId: string; title: string; goalProgressDelta: number; startTime: string }) => [task.routineId, task])),
    });
